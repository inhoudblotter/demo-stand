...
Если вам нужно добавить новую базу данных или пользователя **без удаления существующих данных**:
1. Отредактируйте переменную `POSTGRES_MULTIPLE_DATABASES` в файле `.env`.
2. Обновите конфигурацию контейнера: `docker compose up -d`.
3. Запустите скрипт инициализации вручную:
```bash
docker exec -it demo-postgres bash /docker-entrypoint-initdb.d/init.sh
```
Скрипт проверит существующие базы и создаст только новые, указанные в `.env`.
